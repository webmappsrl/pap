...
export const selectFormJsonState = createFeatureSelector<FormJsonState>(
...
export const selectFormJsonByStep = (step: number) =>
  createSelector(selectFormJsonState, state => state.formJson?.filter(item => item.step === step));
export const selectRequiredFields = createSelector(selectFormJsonState, state =>
