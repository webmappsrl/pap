...
export class PushNotificationEffects {
  ...
  loadPushNotification$ = createEffect(() =>
    this._actions$.pipe(
      ofType(loadPushNotification),
      withLatestFrom(this._store.pipe(select(isLogged))),
      filter(([action, logged]) => logged),
      mergeMap(() =>
        this._pushNotificationSvc.getPushNotification().pipe(
  ...
  removeAllDeliveredNotification$ = createEffect(() =>
    this._actions$.pipe(
      ofType(removeAllDeliveredNotifications),
      withLatestFrom(this._store.pipe(select(isLogged))),
      filter(([action, logged]) => logged),
      mergeMap(() =>
        this._broadcastNotificationSvc.removeAllDeliveredNotifications().pipe(
