...
Registrare il listener `localNotificationActionPerformed` **una sola volta** all'interno di `_initNotifications()`. Il listener emette su `recoveryTap$` al tap di **qualsiasi notifica** (normale o recovery) — l'utente viene sempre portato al calendario.
