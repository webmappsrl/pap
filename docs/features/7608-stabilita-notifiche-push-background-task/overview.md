...
3. **Tap qualsiasi notifica → calendario**: `LocalNotificationService` espone un Observable `recoveryTap$` che emette al tap di qualsiasi notifica (normale o di recupero). `app.component.ts` si sottoscrive e chiama `NavController.navigateRoot('/calendar')`. Separazione netta: il service gestisce le notifiche, il component gestisce la navigazione. Il rescheduling avviene implicitamente tramite il listener `resume` già presente.
...
- [ ] `app.component.ts` si sottoscrive a `recoveryTap$` e chiama `NavController.navigateRoot('/calendar')` — emette per tutte le notifiche, non solo recovery
