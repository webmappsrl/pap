...
before(() => {
  clearTestState();
  // Intercept calendar API before login: app.component dispatches loadCalendars() right after login
  cy.intercept('GET', `${apiCalendar}*`).as('calendarCall');
  cy.visit(Cypress.env('baseurl'));
  e2eLogin();
  cy.wait('@calendarCall').then(xhr => {
    savedCalendarXhr = xhr;
  });
});
...
describe('pap-calendar-page: test the correct behaviour of page', () => {
  it('should navigate to the calendar page and make a successful GET request to the calendar API', () => {
    if (calendarsButton) {
      ...
      cy.wrap(savedCalendarXhr).its('response.statusCode').should('eq', 200);
    } else {
